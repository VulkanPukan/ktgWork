    ...
    public class CustomerRegistrationController : Controller
    {
        ...
        public ActionResult PersonalTrainer()
        {
            ...
            if (customer != null)
            {
                ...
                Session["CustomerType"] = CustomerCategoryType.PersonalTrainer;
                return View();
            }
        ...
        public ActionResult Organization()
        {
            ...
            if (customer != null)
            {
                ...
                Session["CustomerObj"] = customer;
                Session["PersonalPrefix"] = Convert.ToString(ConfigurationManager.AppSettings["PersonalPrefix"]);
                Session["CustomerType"] = CustomerCategoryType.Organization;
        ...
        public ActionResult Register([System.Web.Http.FromBody]CustomerRegistrationViewModel customerModel)
        {
            ...
            {
                Category = customerModel.Category,
                IsSuperAdmin = false,
                IsActive = true,
            ...
            if (cm != null)
            {
                var registrationTemplate = String.Empty;
                var registrationIntimationTemplate = String.Empty;
                var subject = String.Empty;
                switch (customerModel.Category)
                {
                    case (int)CustomerCategoryType.PersonalTrainer:
                        registrationTemplate = "PersonalTrainerRegistration";
                        registrationIntimationTemplate = "PersonalTrainerRegistrationIntimation";
                        subject = "Personal Trainer Registeration";
                        break;
                    case (int)CustomerCategoryType.Organization:
                        registrationTemplate = "OrganizationRegistration";
                        registrationIntimationTemplate = "OrganizationRegistrationIntimation";
                        subject = "Organization Registeration";
                        break;
                }
                mailService.SendMail(customerModel,
                    registrationTemplate, customerModel.ContactEmail, "",
                    "Thankyou for Registration", Server.MapPath("~/MailTemplates"), null);

                mailService.SendMail(customerModel,
                    registrationIntimationTemplate, Convert.ToString(ConfigurationManager.AppSettings["AdminEmail"]), Convert.ToString(ConfigurationManager.AppSettings["AdminEmailCC"]),
                    subject, Server.MapPath("~/MailTemplates"), null);
        ...
        public ActionResult RegisterIndividualAthlete([System.Web.Http.FromBody] UserRegistrationModel customerModel)
        {
            ...
            customer.State = _stateService.ListStates().FirstOrDefault(s => s.ID == customerModel.StateID).Name;
            ...
            if (customerId > 0)
            {
                ...
                user.Sport = sport;
                user.RegisteredSport = customerModel.SportsName;
